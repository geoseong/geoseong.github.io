...
export class Root extends Component {
	constructor(props) {
		...
		this.state = {
			...
			urlPath : this.props.match.url,	/* urlPath */
			pageId: null,
			notebookName : null,
			notebookId : null,
		};
	...
	/* lifecycle method */
	componentDidMount() {
		/* url에 '_' 문자가 있는 위치의 인덱스 */
		let idxOfUnderbar = this.props.match.path.indexOf('_');
		/* url에 '_' 문자가 없는 경우 */
		if(idxOfUnderbar === -1){
			this.setState({
				pageId : '',
				notebookName : this.props.match.path.substring(1),
				notebookId : this.props.storage.routing[this.notebookName],
			})
		}/* url에 '_'문자가 포함된 경우 */
		else{
			this.setState({
				pageId : this.props.match.url.substring(1),
				notebookName : this.props.match.path.substring(1, idxOfUnderbar),
				notebookId : this.props.storage.routing[this.notebookName],
			})
		}
	};
	...
	*/
	...
	renderComponent = (pageId, notebookName, notebookId) => {
		...
		return (
			<div>
				<Helmet>
					<title>Geoseong's Dev Note</title>
					<meta name="description" content="welcome to geoseong's dev note page!" />
					<meta property="og:type" content="article" />
					<link rel="canonical" href="https://geoseong.github.io" />
				</Helmet>
	...
	render() {
		...
		else if(this.props.storage.token.access_token && this.state.urlPath.indexOf('0-') > -1){
		...
		else if(this.props.storage.token.access_token && this.state.urlPath.indexOf('0-') > -1 && this.state.urlPath.indexOf('1-') > -1){
			let ids = this.detectSectionOrPage();
			return (<div>{this.renderComponent(ids.pageContentId, this.notebookName, ids.sectionId)}</div>)
		}/* Reducer에 access_token값이 있으면서 url에 '0-'과 '1-' 둘다 없는 경우 */
		else{
			return (<div>{this.renderComponent(this.state.pageId, this.state.notebookName, this.state.notebookId)}</div>)
		}
	} //end render()
}
