...
const { getMaxCnts, originalRoutings } = require('./routingtest')
...
const appendPost = ({ type, routeKey, endpoint, note, maxCnts }) => {
  ...
  for (let key in originalRoutings) {
    if (key === routeKey) {
      findExists = true
      break
    }
  ...
  if (findExists) {
    validEndpoint =
      originalRoutings[validKey].substring(0, 1) === '/'
        ? originalRoutings[validKey].substring(1)
        : originalRoutings[validKey]
    updatedMaxCnts[endpoint] = maxCnts[endpoint] + 1
  } else if (!findExists && type === 'notebook') {
    validEndpoint = endpoint
    updatedMaxCnts[endpoint] = !maxCnts[endpoint] ? 1 : maxCnts[endpoint] + 1
  } else if (!findExists && type !== 'notebook') {
    // let newPageCnt = !maxCnts[validEndpoint] ? 1 : maxCnts[validEndpoint] + 1
    // validEndpoint = `${endpoint}/${newPageCnt}`
    // updatedMaxCnts[validEndpoint] = newPageCnt
    validEndpoint = endpoint
    updatedMaxCnts[endpoint] = !maxCnts[endpoint] ? 1 : maxCnts[endpoint] + 1
  }
 ...
 * @description this will answer if questioner should recurse or not
 */
